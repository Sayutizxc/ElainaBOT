  ...
  try {
    ...
    const mess = {
      ...
      error: {
        ...
        Ti:
          '[❗] Replay sticker dengan caption *#stickertoimg* atau tag sticker yang sudah dikirim',
    ...
    const ownerNumber = 'xxxxxxxxxx@c.us';
      ...
      for (let i of limit) {
        if (i.id === id) {
          ...
          if (limits >= limitCount) {
            ...
            tobz.reply(
              from,
              'Perintah BOT anda sudah mencapai batas, coba esok hari :)',
              id
            );
            return true;
          } else {
        ...
        if (!isGroupAdmins) {
          return tobz
            ...
            .catch(() =>
              tobz.sendText(
                from,
                `Untung Elaina Bukan Admin, Kalo Jadi Admin Udah Aku Kick Tuh! 😑`
              )
            );
        } else {
    ...
    if (body === '#mute' && isMuted(chatId) == true) {
      if (isGroupMsg) {
        if (!isAdmin)
          return tobz.reply(
            from,
            'Maaf, perintah ini hanya dapat dilakukan oleh admin Elaina!',
            id
          );
        ...
        tobz.reply(
          from,
          'Bot telah di mute pada chat ini! #unmute untuk unmute!',
          id
        );
      } else {
        if (isMsgLimit(serial)) {
    ...
    if (body === '#unbanchat') {
      if (!isOwner)
        return tobz.reply(
          from,
          'Maaf, perintah ini hanya dapat dilakukan oleh Owner Elaina!',
          id
        );
    ...
    if ((isMuted(chatId) && banChat() && !isBlocked && !isBanned) || isOwner) {
      switch (command) {
        ...
        //         return tobz.reply(
        //           from,
        //           `Kirim perintah *#ttg [ Teks ]*, contoh *#ttg aku bukan boneka*`,
        //           id
        //         );
        ...
        case '#stickertoimg':
          if (quotedMsg && quotedMsg.type == 'sticker') {
            ...
            await tobz.sendFile(
              ...
              imageBase64,
              'imagesticker.jpg',
              'Success Convert Sticker to Image!',
              id
            );
          } else if (!quotedMsg)
            return tobz.reply(
              from,
              `Mohon tag sticker yang ingin dijadikan gambar!`,
              id
            );
          break;
        ...
        case '#sgif': // MRHRTZ
          ...
          if ((isMedia && type === 'video') || mimetype === 'image/gif') {
            try {
              ...
              tobz.reply(
                from,
                `Size media terlalu besar! mohon kurangi durasi video.`
              );
            }
          } else if (
            ...
            tobz.reply(
              from,
              `Kesalahan ⚠️ Hanya bisa video/gif apabila file media berbentuk gambar ketik #stickergif`,
              id
            );
          }
          break;
        ...
        case '#sfire':
          ...
          if (isMedia && type === 'image') {
            ...
            await tobz.reply(
              from,
              `Wrong Format!\n⚠️ Harap Kirim Gambar Dengan #stickerfire`,
              id
            );
          }
          break;
...
${desc}`
        ...
        case '#tts':
          ...
          try {
            if (args.length === 1)
              return tobz.reply(
                from,
                'Kirim perintah *#tts [ Bahasa ] [ Teks ]*, contoh *#tts id halo semua*'
              );
            var dataBhs = args[1];
            const ttsHZ = require('node-gtts')(dataBhs);
            var dataText = body.slice(8);
            if (dataText === '')
              return tobz.reply(from, 'Masukkan teksnya', id);
        ...
        case '#antibadword':
          ...
          if (!isBotGroupAdmins)
            return tobz.reply(
              from,
              `Perintah ini hanya bisa di gunakan jika Bot menjadi Admin!`,
              id
            );
          if (args[1] == 'enable') {
            ...
            if (cek) {
              return tobz.reply(
                from,
                `*「 ANTI BADWORD 」*\nPerhatian Untuk Member Grup ${name} Tercinta\nHarap Jangan Toxic Di Sini Atau Elaina Akan Kick!`,
                id
              );
            } else {
        ...
        case '#antilink':
          ...
          if (args[1] == 'enable') {
            ...
            if (cek) {
              return tobz.reply(
                from,
                `*「 ANTI GROUP LINK 」*\nPerhatian Untuk Member Grup ${name} Tercinta\nJika Ingin Send Link Harap Izin Ke Admin`,
                id
              );
            } else {
        ...
        case '#nsfw':
          ...
          if (args[1].toLowerCase() === 'enable') {
            ...
            tobz.reply(
              from,
              'NSFW berhasil di aktifkan di group ini! kirim perintah *#nsfwMenu* untuk mengetahui menu',
              id
            );
        ...
        case '#group':
          ...
          if (args.length === 1)
            return tobz.reply(from, 'Pilih open atau close!', id);
          if (args[1].toLowerCase() === 'open') {
            tobz.setGroupToAdminsOnly(groupId, false);
            tobz.sendTextWithMentions(
              from,
              `Group telah dibuka oleh admin @${sender.id.replace(
                '@c.us',
                ''
            ...
            tobz.sendTextWithMentions(
              from,
              `Group telah ditutup oleh admin @${sender.id.replace(
                '@c.us',
                ''
        ...
        case '#otakudesu':
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              'Kirim perintah *#otakudesu [query]*\nContoh : *#otakudesu darling in the franxx*',
              id
            );
          const animes = await axios.get(
            'https://tobz-api.herokuapp.com/api/otakudesu?q=' + body.slice(11)
          );
        ...
        case '#kusonime':
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              'Kirim perintah *#kusonime [query]*\nContoh : *#kusonime darling in the franxx*',
              id
            );
          const animeq = await axios.get(
            'https://tobz-api.herokuapp.com/api/kuso?q=' + body.slice(7)
          );
        ...
        case '#dewabatch':
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              'Kirim perintah *#dewabatch [query]*\nContoh : *#dewabatch darling in the franxx*',
              id
            );
          const animek = await axios.get(
            'https://tobz-api.herokuapp.com/api/dewabatch?q=' + body.slice(7)
          );
        ...
        case '#komiku':
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              'Kirim perintah *#komiku [query]*\nContoh : *#komiku darling in the franxx*',
              id
            );
          const animep = await axios.get(
            'https://tobz-api.herokuapp.com/api/komiku?q=' + body.slice(7)
          );
        ...
        case '#pinterest':
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              'Kirim perintah *#pinterest [query]*\nContoh : *#pinterest Elaina*',
              id
            );
        ...
        case '#nhview':
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              'Kirim perintah *#nhview [212121]*\nContoh : *#nhview 321421*',
              id
            );
        ...
        case '#loli':
          ...
          const loli = await axios.get(
            `https://tobz-api.herokuapp.com/api/randomloli`
          );
          const loly = loli.data.result;

          tobz.sendFileFromUrl(from, loly, 'loli.jpeg', '*LOLI*', id);
          break;
        case '#shota':
          if (!isGroupMsg)
            return tobz.reply(
        ...
        case '#randomtrapnime':
          ...
          if (trapn.endsWith('.png')) {
            var ext = '.png';
          } else {
            var ext = '.jpg';
          }
          tobz.sendImage(from, trapn, `trapnime${ext}`, 'Trapnime!', id);
          break;
        case '#randomhentai':
          if (!isGroupMsg)
            return tobz.reply(
        ...
        case '#randomanime':
          ...
          const ranimen = ranime.data.result;
          if (ranimen.endsWith('.png')) {
            var ext = '.png';
          } else {
            var ext = '.jpg';
          }
          ...
          try {
            ...
            if (nsfw == true) {
              if (isGroupMsg && isNsfw) {
                ...
                await tobz.reply(
                  from,
                  `Nsfw belum diaktifkan di Grup *${name}*`,
                  id
                );
              }
            } else {
        ...
        case '#wallanime' :
            ...
            const walnimek = walnime.data.url;
        ...
        case '#meme':
          ...
          /* case '#nekopoi':
            ...
            const nekipoi = await axios.get('https://tobz-api.herokuapp.com/api/nekopoi?url=' + body.slice(7) + '&apikey=' + vhtearkey)
...
🌐️ *URL*: ${url}`;
        ...
        case '#jadwalsholat':
          ...
          const jadwalShalat = await axios.get(
            `https://tobz-api.herokuapp.com/api/jadwalshalat?q=${daerah}`
          );
          ...
          const {
            ...
            sunrise,
            sunset,
          } = await jadwalShalat.data.result;
          arrbulan = [
            'Januari',
            'Februari',
            'Maret',
          ...
          await tobz
            .reply(from, resultJadwal, id)
            .catch((e) =>
              tobz.reply(
                from,
                `*Terdapat kesalahan saat mencari jadwal sholat ${args[1]}*`,
                id
              )
            );
          await limitAdd(serial);
        ...
        case '#infogempa':
          ...
          const bmkg = await axios.get(
            'https://tobz-api.herokuapp.com/api/infogempa'
          );
          const {
            potensi,
            koordinat,
        ...
        case '#ssweb':
          ...
          const ssw = await axios.get(
            'https://tobz-api.herokuapp.com/api/url2image?url=' + body.slice(7)
          );
          const ssww = ssw.data;
        ...
        case '#shorturl':
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              'Kirim perintah *#shorturl [linkWeb]*\nContoh : *#shorturl https://neonime.vip*',
              id
            );
        ...
        case '#cuaca':
          ...
          const weather = await axios.get(
            'https://tobz-api.herokuapp.com/api/cuaca?q=' + tempat
          );
        ...
        case '#spamcall':
          ...
          if ((!isOwner, !isAdmin))
            return tobz.reply(
              from,
              'Perintah ini hanya untuk Owner & Admin bot',
              id
            );
          arg = body.trim().split(' ');
          console.log(...arg[1]);
        ...
        case '#ytmp4':
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `Kirim perintah *#ytmp4 [ Link Yt ]*, untuk contoh silahkan kirim perintah *#readme*`
            );
          ...
          try {
            async function ytmp4(){
                tobz.sendText(from,'Tunggu sebentar ya kaka :D')
                ytdl.getInfo(isLin[1]).then(info => {
                if (info.videoDetails.lengthSeconds > 1000){
                    tobz.sendText(from,"Videonya terlalu panjang gan, coba yang lain")
                }else{
                    let stream = ytdl(isLin[1], {
                        quality: 18
                        });
                        ffmpeg(stream)
                        .save(`video/${isLin[1]}.mp4`)
                        .on('end', () => {
                            tobz.sendFile(from,'video/'+ isLin[1] +'.mp4', `${isLin[1]}.mp4`)
                        });
                    limitAdd(serial);
                }
                })
        ...
        //       tobz.reply(
        //         from,
        //         `*Maaf Terdapat kesalahan saat mengambil data, mohon pilih media lain...*`,
        //         id
        //       );
        //     } else {
        ...
        case '#ytmp3':
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              `Kirim perintah *#ytmp3 [ Link Yt ]*, untuk contoh silahkan kirim perintah *#readme*`
            );
          let isLin3 = args[1].match(
            /(?:https?:\/{2})?(?:w{3}\.)?youtu(?:be)?\.(?:com|be)(?:\/watch\?v=|\/)([^\s&]+)/
          );
          
          if (!isLin3) return tobz.reply(from, mess.error.Iv, id);
          try {
            async function ytmp3(){
                tobz.sendText(from,'Tunggu sebentar ya kaka :D')
                ytdl.getInfo(isLin3[1]).then(info => {
                if (info.videoDetails.lengthSeconds > 1000){
                    tobz.sendText(from,"Videonya terlalu panjang gan, coba yang lain")
                }else{
                    let stream = ytdl(isLin3[1], {
                        quality: 'highestaudio',
                        });
                        ffmpeg(stream)
                        .audioBitrate(128)
                        .save(`audio/${isLin3[1]}.mp3`)
                        .on('end', () => {
                            tobz.sendFile(from,'audio/'+ isLin3[1] +'.mp3', `${isLin3[1]}.mp3`)
                        });
                    limitAdd(serial);
                }
                })
        ...
        case '#nhentai': // SEARCH NHENTAI
          ...
          nana
            ...
            .then((g) => {
              ...
              tobz.reply(
                from,
            `*Doujin information*\n*Title* : ${title.pretty}\n*Pages* : ${num_pages}\n*Tags*  : ${tag(tags)}\n*Link* : https://nhentai.net/g/${id}\n\n*Gunakan perintah #nhder ${id} untuk mendownload !*`,
                id
              );
            })
            .catch((err) => {
        ...
        //       'https://tobz-api.herokuapp.com/api/xnxx?url=' + args[1]
        //     );
        ...
        case '#ramalpasangan':
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              'Kirim perintah *#ramalpasangan [kamu|pasangan]*\nContoh : *#ramalpasangan Tobz|Elaina*',
              id
            );
          arg = body.trim().split('|');
          if (arg.length >= 2) {
        ...
        //     tobz.reply(from, mess.wait, id);
        //     const resp = await axios.get(
        //       'https://api.vhtear.com/fbdl?link=' +
        //         body.slice(4) +
        //         '&apikey=' +
        //         vhtearkey
        //     );
        ...
        //   tobz.reply(from, mess.wait, id);
        //   try {
        //     const titkod = await fetch(
        //       `https://api.vhtear.com/tiktokdl?link=${tkdl}&apikey=${vhtearkey}`
        //     );
        //     if (!titkod.ok)
        //       throw new Error(`Error Tiktok : ${titkod.statusText}`);
        ...
        case '#wiki':
          ...
          const wiki = await axios.get(
            `https://tobz-api.herokuapp.com/api/wiki?q=${queryz_.trim()}`
          );
        ...
        case '#kbbi':
          ...
          const kbbl2 = await axios.get(
            `https://tobz-api.herokuapp.com/api/kbbi?kata=${kbbl}`
          );
          ...
          if (argz.length >= 2) {
            ...
            if (!qwery)
              return await tobz.reply(
                from,
                `Kirim perintah *#googleimage [ |Query|Jumlah ]*, contoh = #googleimage |loli|3`,
                id
              );
            if (!jum)
              return await tobz.reply(
                from,
                `Jumlah gambar diperlukan, contoh = #googleimage |loli|3`,
                id
              );
            if (jum >= 5)
              return await tobz.reply(from, 'Jumlah terlalu banyak! Max 4', id);
...
//             return tobz.reply(
//               from,
//               'Kirim perintah *#sandwriting [ Teks ]*\nContoh *#sandwriting Elaina Cantik*',
//               id
//             );
...
//             return tobz.reply(
//               from,
//               'Kirim perintah *#tahta [teks]*\n\nContoh *#tahta elaina*',
//               id
//             );
//           if (jreng.length > 7)
//             return tobz.reply(from, 'Maksimal 7 Huruf!', id);
//           tobz.sendText(
//             from,
...
//             return tobz.reply(
//               from,
//               'Kirim perintah *#resepmasakan [optional]*\nContoh *#resepmasakan rawon*',
//               id
//             );
//           arg = body.trim().split(' ');
//           console.log(...arg[1]);
...
//           break;
        ...
        case '#twtstalk':
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              'Kirim perintah *#twtstalk @username*\nContoh *#twtstalk @miakhalifah*',
              id
            );
          arg = body.trim().split(' ');
          console.log(...arg[1]);
          ...
          try {
            const twstalk2 = await axios.get(
              'https://tobz-api.herokuapp.com/api/twstalk?username=' + twstalk
            );
            const {
              followers_count,
              full_name,
        ...
        case '#igstalk':
          ...
          try {
            const istalk2 = await axios.get(
              'https://tobz-api.herokuapp.com/api/stalk?username=' +
                istalk
            );
            const {
                Biodata,
...
➸ *Nama:* ${Name}
➸ *Bio:* ${Biodata}
➸ *Mengikuti:* ${Jumlah_Following.replace('Following','')}
➸ *Pengikut:* ${Jumlah_Followers.replace('Followers','')}
...
//           break;
        ...
        case '#twitter':
          ...
          if (args.length === 1)
            return tobz.reply(
              from,
              'Kirim perintah *#twitter [linkTwitter]* untuk contoh silahkan kirim perintah *#readme*'
            );
          arg = body.trim().split(' ');
          console.log(...arg[1]);
          var slicedArgs = Array.prototype.slice.call(arg, 1);
          ...
          try {
            const twtdl2 = await axios.get(
              'https://tobz-api.herokuapp.com/api/twit?url=' + twtdl
            );
...
➸ *Wilayah:* +${region_name}`;
        ...
        case '#nulis':
          ...
          const ngetikk = await axios.get(
            'https://bangandre.herokuapp.com/nulis?teks=' +
              ngettik
          );
        ...
        default:
          if (!isGroupMsg)
            return tobz.reply(
              from,
              'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup Elaina, Link Ada Di Bio atau Bisa Mengetik #elainagroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*',
              id
            );
